...
function do_email(url) {	
	if (get_selected_values().length > 0) {
		$('#delete, #generate_barcodes, #email').removeClass('disabled').prop('disabled', false).removeAttr('disabled');
	} else {
		$('#delete, #generate_barcodes, #email').addClass('disabled').prop('disabled', true).attr('disabled', 'disabled');
	}

	if (get_selected_values().length > 1) {
  	$('#bulk_edit').removeClass('disabled').prop('disabled', false).removeAttr('disabled');
  } else {
  	$('#bulk_edit').addClass('disabled').prop('disabled', true).attr('disabled', 'disabled');	
  }

	//If email is not enabled, don't do anything
	if(!enable_email.enabled) return;
...
function get_table_row(id) {
	...
	if ($element.length === 0) {
		$element = $("#sortable_table a[data-url*='" + id + "']");
	}
	return $element;
}
...
(function($) {
	$(function() {
		$('#table_holder').on('change', '[type=checkbox]', function(evt) {
      if (get_selected_values().length > 0) {
        $('#delete, #generate_barcodes, #email').removeClass('disabled').prop('disabled', false).removeAttr('disabled');
      } else {
        $('#delete, #generate_barcodes, #email').addClass('disabled').prop('disabled', true).attr('disabled', 'disabled');
