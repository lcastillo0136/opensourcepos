...
class Item extends CI_Model
{
	...
	*/
	...
	function get_all_export($stock_location_id=-1, $limit=1000, $offset=0) {
		$all_stocks = "";
		$stock_locations = $this->Stock_locations->get_undeleted_all()->result_array();          
	  foreach($stock_locations as $location_data) {
	  	$all_stocks.= ",' ' as " . $location_data["location_name"] . ", ' ' as ". $location_data["location_name"] . "_val";
	  }

		$this->db->select('item_number, name, category, supplier_id, cost_price, unit_price, \' \' as tx_name1, \' \' as tx_val1, \' \' as tx_name2, \' \' as tx_val2, reorder_level, description, allow_alt_description, is_serialized, custom1, custom2, custom3, custom4, custom5, custom6, custom7, custom8, custom9, custom10'.$all_stocks.',item_id', FALSE);
		...
		foreach ($query->result() as $row) {
			$CI = &get_instance();
			$tax_info = $CI->Item_taxes->get_info($row->item_id);
			if (count($tax_info) > 0) {
				if (isset($tax_info[0])) {
					$row->tx_name1 = $tax_info[0]["name"];
					$row->tx_val1 = $tax_info[0]["percent"];
				}

				if (isset($tax_info[1])) {
					$row->tx_name2 = $tax_info[1]["name"];
					$row->tx_val2 = $tax_info[1]["percent"];
				}
			}	

			foreach($stock_locations as $location_data) {
				$item_quantity = $CI->Item_quantities->get_item_quantity($row->item_id, $location_data['location_id']);
				$row->{$location_data["location_name"]} = $location_data["location_id"];
				$row->{$location_data["location_name"]."_val"} = $item_quantity->quantity;
			}
	...
	function get_all_filtered($stock_location_id,$is_serialized=0,$no_description,$search_custom,$is_deleted) { 
		/**GARRISON MODIFIED 4/21/2013, Parq 131215 **/
		$this->db->from('items');
		if ($stock_location_id > -1)
		{
			$this->db->join('item_quantities','item_quantities.item_id=items.item_id');
		...
		$this->db->where('deleted', $is_deleted ? $is_deleted : 0);
		$this->db->order_by("name", "asc");
		return $this->db->get();
	}
	...
	*/
	function get_search_suggestions($search, $limit=25) {
		...
		$this->db->from('items');
    $this->db->where("(name like '%$search%' or item_number like '%$search%' or description like '%$search%')");
		$this->db->where('deleted',0);
		$this->db->order_by("name", "asc");
		$by_name = $this->db->get();
