...
{	
	...
	*/
	...
	function get_all_export($limit=1000, $offset=0) {
		$this->db->select('first_name,last_name,email, phone_number,address_1,address_2,city,state,zip,country,comments,account_number,taxable');
		$this->db->from('customers');
		$this->db->join('people','customers.person_id=people.person_id');			
		$this->db->where('deleted',0);
		$this->db->order_by("last_name", "asc");
	...
	*/
	function get_customer_search_suggestions($search,$limit=25)
	{
		...
		$this->db->join('people','customers.person_id=people.person_id');	
		$this->db->where('deleted',0);	
		$this->db->where("(first_name LIKE '%$search%' or last_name LIKE '%$search%' or CONCAT(`first_name`,' ',`last_name`) LIKE '%$search%' or account_number like '%$search%')");
		$this->db->order_by("last_name", "asc");		
		$by_name = $this->db->get();
		foreach($by_name->result() as $row)
		{
