...
/** GARRISON MODIFIED 4/24/2013 **/	
	...
	*/
	function search($search) {
		...
		$this->db->where("(first_name like '%" . $this->db->escape_like_str($search) . "%' or last_name like '%" . $this->db->escape_like_str($search) . "%' or CONCAT(`first_name`,' ',`last_name`) like '%" . $this->db->escape_like_str($search) . "%' or giftcard_number like '%" . $this->db->escape_like_str($search) . "%')");
		$this->db->where('deleted',0);
		$this->db->order_by("giftcard_number", "asc");
		return $this->db->get();
	}
